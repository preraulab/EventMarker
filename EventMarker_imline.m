classdef EventMarker_imline < handle
    ...
    methods (Access = public)
        ...
        function obj = EventMarker_imline(varargin)
        ...
        function mark_event(obj, event_ID, position)
            ...
            %Create event ID (add 10000 so it is not confused with event type ID)
    ...
    methods (Access = private)
            ...
            if isempty(obj.event_list)
                ...
                if isempty(obj.current_object)
                    ...
                    if closest_obj.region
                        ...
                        setPositionConstraintFcn(selected_obj,@(pos)obj.constrain_rect(pos,obj.xbounds,obj.ybounds, closest_obj.constrain));
                        %Update the event text when region is moved
                        addNewPositionCallback(selected_obj,@(src,event)obj.updateeventtext(selected_obj, label_handle, true));
                        ...
                        setPositionConstraintFcn(selected_obj,@(pos)obj.constrain_line(pos,obj.xbounds,obj.ybounds));
                        %Update the text label
                        addNewPositionCallback(selected_obj,@(src,event)obj.updateeventtext(selected_obj, label_handle, false));
        ...
        %-----------------------------------------------------------
        function updateeventtext(~, imobject, label_handle, region)
        ...
        %-----------------------------------------------------------
        function new_pos=constrain_rect(~, pos, xbounds, ybounds, constrain)
            %New position
            new_pos = pos;
